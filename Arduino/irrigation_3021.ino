...
void loadConfiguration()
{
  ...
  /*EEPROM.get(0,setPointHumedad);
    if(setPointHumedad==0)
    {
       EEPROM.put(0,defaultHumedad);
       Serial.println("saving default EEPROM ");
       Serial.println(setPointHumedad);
    }
    else
    {
       Serial.print("reading humidity from EEPROM ");
       Serial.println(setPointHumedad);
    }

    EEPROM.get(1,setPointNivelAgua);
    if(setPointNivelAgua==0)
    {
       EEPROM.put(1,defaultNivel);
       Serial.println("saving default EEPROM ");
       Serial.println(setPointNivelAgua);
    }
    else
    {
       Serial.print("reading Nivel Agua from EEPROM ");
       Serial.println(setPointNivelAgua);
    }

    EEPROM.get(2,setPointLluvia);
    if(setPointLluvia==0)
    {
       EEPROM.put(2,defaultLluvia);
       Serial.println("saving default EEPROM ");
       Serial.println(setPointLluvia);
    }
    else
    {
       Serial.print("reading Nivel Agua from EEPROM ");
...
void loop()
{
  ...
  ***************************************************/
  ...
  if (Modo == 3) //Normal Mode / Modo Normal
  {
    ...
    {
      sprintf(buffer, "Time--> %02d:%02d:%02d", clock.getHour(h12Flag, pmFlag)
              , clock.getMinute()
  ...
  else if (Modo == 2) //modo test
  {
    ...
    Serial.print("Rain Intensitivity Value : ");
  ...
  else if (Modo == 1) //modo setup: cambia setpoints, humedad,nivel, lluvia
  {
    ...
    lcd.setCursor(8, 1);
    lcd.print("R: "); lcd.print(sensorValueLluvia);
    //delay(3000);
    // lcd.noBacklight();
  }
  ...
  else if (Modo == 99) //check new second value from DS3231
  {
    ...
    if (openSeconds <=0)
    {
     ...
     {
          sprintf(buffer,"Remain>  %-3u sec",openSeconds);     
            

          lcd.setCursor(0, 0);
          lcd.print(buffer);
          
          sprintf(buffer, "Time-->  %02d:%02d  ", clock.getHour(h12Flag, pmFlag)
                                                            , clock.getMinute());
                                                            //, clock.getSecond());        
